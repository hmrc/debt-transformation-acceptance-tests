...
trait BaseRequests extends RandomValues with BaseUris {
  ...
  def createBearerToken(enrolments: Seq[String] = Seq(), userType: String="userType", utr: String="123456789012") ={
  ...
  def createLocalBearerToken(enrolments: Seq[String] = Seq(), userType: String = getRandomAffinityGroup,utr: String = "123456789012"): String = {
    val json = Json.obj(
    ...
    val authHeader: (String, Seq[String]) = response.headers
      .filter(header => header._1.equalsIgnoreCase("Authorization")).head
