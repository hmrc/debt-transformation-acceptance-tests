...
class FCMultipeDebtItemsFeatureSpec
  ...
  Feature("FC Debt Calculation End point testing") {
    ...
    ignore("7. Total Payments cannot be 0.") { context =>
      ...
      Then(
        "the fc ifs service will respond with Could not parse body due to requirement failed: Payment amount must not be zero"
      )
    ...
    ignore("8. Total Payments cannot be negative.") { context =>
      ...
      Then(
        "the fc ifs service will respond with Could not parse body due to requirement failed: Payment amount must be positive"
      )
      // TODO: No matching helper method found for this step. Validate and call the correct helper.
    ...
    ignore("9. Total Payment amounts cannot be more than the original amount.") { context =>
      ...
      Then(
        "the fc ifs service will respond with Could not parse body due to requirement failed: Total Payment amounts cannot be more than the original amount"
      )
      // TODO: No matching helper method found for this step. Validate and call the correct helper.
    ...
    ignore("FC 2 Debts - Interest rate changes from 3% to 3.25% and then multiple payments are made for both debts") {
      context =>
        Given("a fc debt item")
        // TODO: Helper 'aFcDebtItem' expects context 'InterestForecastingContext' but this spec uses 'FCStatementOfLiabilityContext'.
        // Validate whether this scenario should use a different context or whether the helper should be aligned to this spec context.
    ...
    ignore("periodEnd and interestStartDate is missing or invalid.") { context =>
      ...
      Then(
        "the fc ifs service will respond with Field at path /debtItems(0)/periodEnd missing or invalid\\nField at path /debtItems(0)/interestStartDate missing or invalid"
      )
      // TODO: No matching helper method found for this step. Validate and call the correct helper.
