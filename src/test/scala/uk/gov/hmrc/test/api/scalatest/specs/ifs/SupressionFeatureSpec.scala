...
import uk.gov.hmrc.test.api.models.{CalculationWindow, DebtCalculation, DebtCalculationsSummary, SuppressionApplied, SuppressionInformation, SuppressionRequest}
...
class SupressionFeatureSpec
  ...
  Feature("Suppression") {
    ...
    Scenario(
      "Suppression applied by all criteria on a single debt item.",
      DTD_3325
    ) { context =>
      val suppressionContext = SuppressionRulesContext()
